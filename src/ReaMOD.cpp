...
bool StyledSliderDouble(
    ...
    ImGui::PushStyleColor(ctx, ImGui::Col_Text, blue);
    bool changed = ImGui::SliderDouble(ctx, label, value, min, max, format, flags);
    ImGui::PopStyleColor(ctx);
    ...
    if (ImGui::Begin(reaMOD_Main_ImGui_Context, "ReaMOD Window", &open, ImGui::WindowFlags_NoFocusOnAppearing)) {
            ...
            for (auto& group : otherGroups) {
                ...
                if (ImGui::TreeNode(reaMOD_Main_ImGui_Context, groupName.c_str())) {
                    ...
                    for (auto& globalParam : parameters) {
                        ...
                        // Use a styled slider to create a slider for the parameter
                        StyledSliderDouble(reaMOD_Main_ImGui_Context, globalParam.name.c_str(), &doubleValue, doubleMin, doubleMax);
        ...
        if (!selectedFMODEvent.empty()) {
            ...
            for (auto& param : selectedEventParameters) {
                ...
                // Use a styled slider to create a slider for the parameter
                StyledSliderDouble(reaMOD_Main_ImGui_Context, param.name.c_str(), &doubleValue, doubleMin, doubleMax);
